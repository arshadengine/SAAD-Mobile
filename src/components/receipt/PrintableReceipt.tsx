...
export const PrintableReceipt: React.FC<PrintableReceiptProps> = ({
  ...
  const renderPaymentCheckbox = (label: string, isChecked: boolean) => (
    <div className="flex items-center gap-1.5 cursor-default">
      <div className={`w-3.5 h-3.5 border border-[#947844] rounded-sm flex items-center justify-center ${isChecked ? 'bg-[#785412] border-[#785412]' : 'bg-white'}`}>
        {isChecked && (
  ...
  return (
    <div className="space-y-4 select-none">
      {/* ======================================================== */}
      {/* SCREEN-ONLY VIEW SWITCHER (Tabs: Page 1, Page 2, Both)   */}
      {/* ======================================================== */}
      {isPreview && (
        <div
          data-html2canvas-ignore="true"
          className="print:hidden bg-white border border-slate-250 p-2 rounded-xl shadow-sm flex flex-wrap items-center justify-between gap-2 max-w-2xl mx-auto select-none"
        >
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider pl-1">Page View:</span>
            <button
              type="button"
            ...
            >
              <span>📜</span>
              <span>Page 2 (Terms & Conditions)</span>
            </button>
            <button
              type="button"
          ...
          <div className="text-[11px] font-semibold text-slate-500 pr-2">
            {activeTab === 'page1' ? 'Showing Page 1' : activeTab === 'page2' ? 'Showing Page 2' : 'Showing 2 Separate Pages'}
          </div>
        </div>
      )}

      {/* ======================================================== */}
      {/* PRINTABLE RECEIPT ROOT CONTAINER (Captured by PDF/Print)  */}
      {/* ======================================================== */}
      <div
        id="printable-receipt-content"
        className={`mx-auto font-sans leading-tight select-none ${
          isPreview ? 'max-w-2xl' : 'w-full max-w-2xl'
      ...
      >
        ...
        {/* ======================================================== */}
        <div
          className={`receipt-page receipt-page-1 bg-white text-slate-900 border border-slate-300 shadow-md ${
            isPreview ? 'rounded-sm mb-6' : 'border-none shadow-none mb-0'
          } ${activeTab === 'page2' && isPreview ? 'hidden print:block' : 'block'}`}
          style={{ boxSizing: 'border-box' }}
        >
          {/* Top Warm Champagne & Gold Header Banner */}
          <div className="bg-[#fdfaf3] text-slate-900 py-3 px-5 relative border-b-2 border-[#d4af37] border-t-4 border-t-[#d4af37]">
            <div className="flex flex-row items-center gap-5">
              ...
              <div className="shrink-0">
                {shopSettings.logoUrl ? (
                  <img
                    ...
                    className="h-20 w-20 object-contain rounded-full border-2 border-[#d4af37] bg-white p-0.5 shadow-sm"
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = '/logo.png';
                    }}
                  />
                ) : (
                  <div className="w-20 h-20 rounded-full border-2 border-[#d4af37] bg-[#f8f1e0] flex flex-col justify-center items-center p-2 text-center shadow-sm">
                    <span className="text-[#785412] font-black text-lg leading-none">SAAD</span>
                    <span className="text-slate-800 text-[10px] font-extrabold tracking-widest mt-1">MOBILE</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 text-center pr-6">
                <h1 className="text-3xl font-black tracking-wider text-slate-900 uppercase leading-none">
                  SAAD <span className="text-[#b48316]">MOBILE</span>
                </h1>
                
                <div className="text-[10px] font-bold tracking-widest text-[#785412] uppercase mt-2 flex items-center justify-center gap-2 w-full">
                  <span className="h-[1px] flex-1 bg-[#d4af37]/60 max-w-[80px]"></span>
                  <span className="whitespace-nowrap">{shopSettings.tagline || 'MOBILE SALES & SERVICES'}</span>
                  <span className="h-[1px] flex-1 bg-[#d4af37]/60 max-w-[80px]"></span>
                </div>

                {/* Service Badges */}
                <div className="flex flex-wrap justify-center items-center gap-x-4 gap-y-1 text-[10px] text-slate-700 mt-2.5 font-bold">
                ...
                <div className="flex flex-row justify-between items-start text-[10px] text-slate-650 mt-3 pt-2 border-t border-[#e5d8c3] gap-4">
                  <div className="flex items-start gap-1 text-left max-w-[280px]">
                    <span className="text-[#b48316] shrink-0 font-bold">📍</span>
                    <span className="text-slate-800 font-medium">{shopSettings.address || 'Shop No. 4, Opp. Bus Stand, Main Road, Katraj, Pune - 411046'}</span>
                  </div>
                  <div className="flex flex-col items-end text-right space-y-1 shrink-0 font-mono">
                    <div className="flex items-center gap-1">
                      <span className="text-[#b48316]">📞</span>
                      <span className="text-slate-900 font-bold">+91 {shopSettings.phone || '98765 43210'}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-emerald-700 font-bold">💬</span>
                      <span className="text-slate-900 font-bold">+91 {shopSettings.whatsapp || shopSettings.phone || '98765 43210'}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="p-4 bg-slate-50/60 space-y-3.5">
            ...
            {/* Bill Meta Data */}
            <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-xs font-semibold text-slate-800 border-b border-dashed border-slate-300 pb-3">
            ...
            <div className="border-t border-b border-dashed border-slate-300 py-2.5 text-xs">
              <div className="bg-[#f3ebd7] text-[#785412] border border-[#d4af37]/70 px-3 py-1 font-black uppercase tracking-wider text-[10px] rounded-sm inline-block mb-2 select-none shadow-xs">
                <span>👥</span>
            ...
            <div className="space-y-3">
              <div className="bg-[#f3ebd7] text-[#785412] border border-[#d4af37]/70 px-3 py-1 font-black uppercase tracking-wider text-[10px] rounded-sm inline-block select-none shadow-xs">
                <span>📱</span>
              ...
              <div className="text-xs">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#f3ebd7] text-[#785412] text-[11px] font-black uppercase border border-[#d4af37]/70 select-none">
                      <th className="py-2.5 px-2 border-r border-[#d4af37]/50 text-center w-8">#</th>
                      <th className="py-2.5 px-3 border-r border-[#d4af37]/50">ITEM DETAILS</th>
                      <th className="py-2.5 px-3 border-r border-[#d4af37]/50">IMEI / WARRANTY</th>
                      <th className="py-2.5 px-2 border-r border-[#d4af37]/50 text-center w-12">QTY</th>
                      <th className="py-2.5 px-3 border-r border-[#d4af37]/50 text-right w-28">PRICE (₹)</th>
            ...
            <div className="grid grid-cols-2 gap-6 items-start pt-1">
              ...
              <div>
                <div className="bg-[#f3ebd7] text-[#785412] px-3 py-1.5 font-black uppercase tracking-wider text-[10px] rounded-t flex items-center gap-1.5 border border-[#d4af37]/70 border-b-0 select-none shadow-xs">
                  <span>💳</span>
                ...
                <div className="border border-slate-300 border-t-0 p-4 bg-white rounded-b space-y-1.5">
                  ...
                  <div className="bg-[#fbf4e2] border-2 border-[#d4af37] text-slate-900 px-3 py-2 rounded flex justify-between items-center mt-3 shadow-xs select-none">
                    <span className="font-extrabold text-[11px] tracking-wider uppercase text-[#785412]">TOTAL AMOUNT</span>
              ...
              <div className="flex flex-col justify-between h-full space-y-3">
                ...
                <div className="flex justify-between items-end pt-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-20 h-20 border border-slate-300 p-0.5 bg-white flex justify-center items-center rounded-sm shrink-0 shadow-xs">
                      <img
            ...
            <div className="bg-[#fef9ee] border border-[#e5d5ad] rounded p-2 flex items-center justify-between text-[10px] text-[#785412] font-semibold select-none shadow-2xs">
              <div className="flex items-center gap-2">
                <span className="text-amber-600 text-sm">📋</span>
                <span>नियम व शर्तें (Terms & Conditions, No-Return Policy & Customer Declaration) पृष्ठ 2 पर देखें।</span>
              </div>
              <span className="bg-[#f3ebd7] text-[#785412] text-[9px] font-extrabold px-2 py-0.5 rounded border border-[#d4af37]/70 tracking-wider shrink-0">
                PAGE 2 →
              </span>
            </div>
          </div>
          ...
          <div className="bg-[#f8f3e8] text-slate-700 px-3 py-2 flex justify-around items-center text-[9px] font-bold border-t-2 border-[#d4af37] select-none">
            <div className="flex items-center gap-1.5">
              <span className="text-[#b48316] text-xs">✔</span>
              <span>100% Genuine Products</span>
            </div>
            <div className="h-3 w-[1px] bg-[#d4af37]/40"></div>
            <div className="flex items-center gap-1.5">
              <span className="text-[#b48316] text-xs">💎</span>
          ...
          <div className="bg-white border-t border-slate-200 py-1.5 px-4 font-bold text-[10px] text-slate-800 tracking-widest select-none flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-[#b48316]">★★★</span>
        ...
        {/* ======================================================== */}
        {/* SCREEN-ONLY VISUAL PAGE SEPARATOR (Ignored by PDF/Print) */}
        {/* ======================================================== */}
        {isPreview && activeTab === 'both' && (
          <div
            data-html2canvas-ignore="true"
            className="print:hidden my-6 flex items-center justify-center gap-3 select-none"
          >
            <div className="h-[1px] bg-[#d4af37]/40 flex-1"></div>
            <div className="bg-[#f3ebd7] text-[#785412] text-xs font-bold px-4 py-1.5 rounded-full shadow-sm flex items-center gap-2 border border-[#d4af37]/70">
              <span>📜</span>
        ...
        {/* ======================================================== */}
        <div
          className={`receipt-page receipt-page-2 bg-white text-slate-900 border border-slate-300 shadow-md ${
            isPreview ? 'rounded-sm' : 'border-none shadow-none'
          } ${activeTab === 'page1' && isPreview ? 'hidden print:block' : 'block'}`}
          style={{ boxSizing: 'border-box', pageBreakBefore: 'always', breakBefore: 'page' }}
        >
          {/* Page 2 Top Warm Champagne & Gold Header Banner */}
          <div className="bg-[#fdfaf3] text-slate-900 py-3 px-5 border-b-2 border-[#d4af37] border-t-4 border-t-[#d4af37] select-none">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                {shopSettings.logoUrl ? (
                  <img
                    src={shopSettings.logoUrl}
                    alt={shopSettings.shopName}
                    className="h-12 w-12 object-contain rounded-full border-2 border-[#d4af37] bg-white p-0.5 shadow-sm"
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = '/logo.png';
                    }}
                  />
                ) : (
                  <div className="w-12 h-12 rounded-full border-2 border-[#d4af37] bg-[#f8f1e0] flex flex-col justify-center items-center text-center shadow-sm">
                    <span className="text-[#785412] font-black text-xs leading-none">SAAD</span>
                    <span className="text-slate-800 text-[8px] font-extrabold tracking-widest mt-0.5">MOBILE</span>
                  </div>
                )}
                <div>
                  <h2 className="text-lg font-black tracking-wider text-slate-900 uppercase leading-none">
                    SAAD <span className="text-[#b48316]">MOBILE</span>
                  </h2>
                  <div className="text-[9px] text-[#785412] font-bold tracking-wider uppercase mt-1">
              ...
              <div className="text-right text-[10px] text-slate-700 space-y-0.5 font-mono">
                <div>
                  <span className="text-slate-500">Invoice:</span> <span className="font-bold text-[#b48316]">{receipt.billNumber}</span>
                </div>
                <div>
                  <span className="text-slate-500">Date:</span> <span className="text-slate-900 font-semibold">{formatInvoiceDate(receipt.date)}</span>
                </div>
                <div className="font-sans text-[10px]">
                  <span className="text-slate-500">Customer:</span> <span className="text-slate-900 font-bold">{receipt.customerName || 'Walk-in Customer'}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 bg-slate-50/60 space-y-3.5">
            ...
            <div className="border border-slate-300 rounded-lg overflow-hidden bg-white shadow-sm">
              <div className="bg-[#f3ebd7] text-[#785412] px-3.5 py-1.5 font-bold uppercase tracking-wider text-[11px] flex items-center justify-between border-b border-[#d4af37]/60 select-none">
                <div className="flex items-center gap-2">
                  <span>📱</span>
                  <span>सेकंड हैंड मोबाइल – नो रिटर्न, नो गारंटी पॉलिसी</span>
                </div>
                <span className="text-[9px] bg-[#fef3c7] text-[#92400e] px-2 py-0.5 rounded border border-[#f59e0b]/40 font-extrabold">
                  अनिवार्य नियम
                </span>
              </div>
              ...
              <div className="p-3 text-[11px] leading-relaxed text-slate-800 space-y-2 font-medium">
                <div className="flex gap-2 items-start">
                  <span className="w-5 h-5 rounded-full bg-[#f3ebd7] text-[#785412] font-black text-[10px] flex items-center justify-center shrink-0 mt-0.5 border border-[#d4af37]/70">1</span>
                  <span className="pt-0.5 font-semibold text-slate-900">यह मोबाइल सेकंड हैंड (Used Mobile) है।</span>
                </div>

                <div className="flex gap-2 items-start">
                  <span className="w-5 h-5 rounded-full bg-[#f3ebd7] text-[#785412] font-black text-[10px] flex items-center justify-center shrink-0 mt-0.5 border border-[#d4af37]/70">2</span>
                ...
                <div className="flex gap-2 items-start">
                  <span className="w-5 h-5 rounded-full bg-[#f3ebd7] text-[#785412] font-black text-[10px] flex items-center justify-center shrink-0 mt-0.5 border border-[#d4af37]/70">3</span>
                ...
                <div className="flex gap-2 items-start">
                  <span className="w-5 h-5 rounded-full bg-[#f3ebd7] text-[#785412] font-black text-[10px] flex items-center justify-center shrink-0 mt-0.5 border border-[#d4af37]/70">4</span>
                ...
                <div className="flex gap-2 items-start">
                  <span className="w-5 h-5 rounded-full bg-[#f3ebd7] text-[#785412] font-black text-[10px] flex items-center justify-center shrink-0 mt-0.5 border border-[#d4af37]/70">5</span>
            ...
            <div className="border border-slate-300 rounded-lg overflow-hidden bg-white shadow-sm">
              <div className="bg-[#f3ebd7] text-[#785412] px-3.5 py-1.5 font-bold uppercase tracking-wider text-[11px] flex items-center gap-2 border-b border-[#d4af37]/60 select-none">
                <span>🛡️</span>
            ...
            <div className="border border-[#d4af37]/70 rounded-lg p-3 bg-[#fdfbf7] space-y-1.5 shadow-2xs">
              <div className="flex items-center gap-2 text-[#785412] font-extrabold text-[11px] uppercase tracking-wide">
                <span>✍️</span>
